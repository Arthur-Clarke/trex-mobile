...
function setup() {
  ...
  trex.scale = 0.5;
  solo = createSprite(width/2,170,width,20);
  ...
  soloInvisivel =  createSprite(width/2,200,width,20);
...
function draw(){
  ...
  background("white");
  text("Pontuação "+ pontuacao, width-150, 50);
...
function gerarNuvens(){
  if(frameCount % 60 == 0){
    ...
    //tempo de vida
    nuvem.lifetime = Math.round (width/-nuvem.velocityX);
...
function gerarObstaculos(){
  if(frameCount % 60 == 0){
    obstaculo = createSprite(width,165,10,40);
    ...
    obstaculo.scale = 0.5;
    obstaculo.lifetime = Math.round(width/-obstaculo.velocityX);
